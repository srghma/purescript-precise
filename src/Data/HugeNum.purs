...
dropZeroes = dropIntegralZeroes <<< dropFractionalZeroes
  where
  dropFractionalZeroes (HugeNum r) = HugeNum z
    where
  ...
  dropIntegralZeroes (HugeNum r) = HugeNum z
    where
    integralPart = L.take r.decimal r.digits
    zeroes = L.takeWhile (_ == _zero) integralPart
    digits =
      if L.length integralPart == L.length zeroes then pure _zero
...
equivalize = integralize <<< fractionalize
  where
...
fromString s = do
  ...
  let
    signDigits = case leadingChar of
...
floatToHugeNum n = HugeNum r
  where
  pos = n >= zero
  split =
    if pos then L.span (_ /= '.') (L.fromFoldable <<< toCharArray $ show n)
...
integralToHugeNum n =
  let
    integral = L.fromFoldable <<< toCharArray $ show n
  ...
  in
    HugeNum
      { digits: L.mapMaybe fromChar integral <> fractional
      , decimal
      , sign
      }

scientificToHugeNum :: Number -> HugeNum
scientificToHugeNum n = HugeNum r
  where
...
parsePlusMinus exp base =
  let
    r = parsePlusPlus exp base
  in
    r { sign = Minus, digits = r.digits <> pure _zero }

parseMinusPlus :: Int -> List Char -> HugeRec
parseMinusPlus exp base = r
  where
...
truncate n (HugeNum r) = HugeNum z
  where
...
round h
  | abs (h - floor h) < abs (ceil h - h) = floor h
  | otherwise = ceil h
...
addPlusPlus x y = dropZeroes (HugeNum z)
  where
...
addMinusMinus x y =
  let
    z = rec (addPlusPlus x y)
  in
    HugeNum (z { sign = Minus })

-- | Assumes 0 <= x, 0 <= abs y <= x, y <= 0
addPlusMinus :: HugeNum -> HugeNum -> HugeNum
addPlusMinus x y = (HugeNum z)
  where
  eqv = equivalize { fst: x, snd: y }
...
adjustDecimalForFrontZeroes xs oldDec =
  let
    newDigits' = unsafeRemoveFrontZeroes $ L.take oldDec xs
...
plus x y
  ...
  | otherwise = z
      ...
      lesser = min x y
      z =
        if greater == greaterMag then addPlusMinus greater lesser
...
timesPowOfTen n (HugeNum r) = z
  where
  newDecimal = r.decimal + n
  newZeroesLength =
    if newDecimal >= L.length (takeMeatyParts r.digits) then newDecimal - L.length (takeMeatyParts r.digits)
    else 0
...
meatyDecimals (HugeNum r) =
  let
    decimals = L.reverse $ L.drop r.decimal r.digits
...
makeHugeInteger' :: HugeNum -> HugeNum
makeHugeInteger' (HugeNum r) = HugeNum z
  where
...
adjustDecimalForTriviality h1 h2 (HugeNum r3) = dropZeroes (HugeNum r)
  where
